...
def main() -> None:
    ...
    for motor, port_name, info in motors:
        motor_feedback = motor.get_latest_feedback()
        for motor_id in info.keys():
            all_motors.append((motor, port_name, motor_id))
            # motor.set_position(motor_id, motor_feedback[motor_id].position)
    ...
    try:
        while True:
            ...
            elif motor_inits[active_motor_index]:
                motor_feedback = active_motor.get_latest_feedback()
                if len(motor_feedback) > 0:
                    print(motor_feedback)
                    motor_inits[active_motor_index] = False
                    active_motor.set_position(motor_id, motor_feedback[motor_id].position)
                    time.sleep(0.1)
                    active_motor.set_kp(motor_id, 80.0)
                    active_motor.set_kd(motor_id, 10.0)
